...
ZipFile *processArchive(char *path) {
    ...
    unsigned char *data, *pntr, *data2;
    int entries, fd, len, len2;

    HashTable *hash_table;
    ZipFile *zip;

    //fprintf(stderr, ">>> processArchive : %s\n", path);
    ...
    if((data = (unsigned char*)malloc(len)) == 0)
        goto error;
    lseek(fd, 0, SEEK_SET);
    data2 = data;
    while (1)
    {
		len2 = read(fd, data2, len);
		if (len2 == 0)
		{
			break;
		}
    ...
    while(entries--) {
        ...
        //fprintf(stderr, ">>>> processArchive pathname : %s\n", pathname);
...
error2:
    fprintf(stderr, ">>>> processArchive error2\n");
#if !defined(__MINGW32__) 
    munmap(data, len);
#else
	free(data);
...
error:
    fprintf(stderr, ">>>> processArchive error\n");
    close(fd);
    return NULL;
}
...
char *findArchiveDirEntry(char *pathname, ZipFile *zip) {
    ...
    //fprintf(stderr, ">>> findArchiveDirEntry : %s\n", pathname);
...
char *findArchiveEntry(char *pathname, ZipFile *zip, int *uncomp_len) {
    ...
    //fprintf(stderr, ">>> findArchiveEntry : %s\n", pathname);
