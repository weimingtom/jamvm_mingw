...
#define UNARY_MINUS(TYPE)               \
    ...
    DEF_OPC_012(OPC_L2I, {
        ...
        //fprintf(stderr, "<<<<<<<<<<< L2I cache %ld\n", value);
#else


#if 1
		int64_t value = STACK_POP(int64_t);
		//fprintf(stderr, "<<<<<<<<<<< L2I nocache %ld\n", value);
#else
		//for debug
        ...
        //41F7FAC0
        ostack -= ((SLOTS(int64_t)));
        int64_t value = *(int64_t*)&ostack[0 * 2]; //why mingw +1???

#if 0
        int i;
        for (i = -10; i <= 10; ++i)
        {
			fprintf(stderr, "<<<<<<<<<<< L2I [%d]== %X\n", i, ostack[i]);
        }
        fprintf(stderr, "<<<<<<<<<<< L2I nocache %X, %X\n", ostack[0], 421);
        fprintf(stderr, "<<<<<<<<<<< L2I nocache %X, %X\n", ostack[1], 421);
        //see nativeExtraArg, bad callJNIMethod return type 
#endif //for debug
#endif
